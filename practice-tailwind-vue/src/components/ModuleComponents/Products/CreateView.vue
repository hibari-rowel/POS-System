...
const submitForm = async () => {
    ...
    if (confirmation.isConfirmed) {
        ...
        let isSuccessfull = await productStore.createProduct(form, rules);

        Swal.close();

        if (isSuccessfull) {
            fireToast("success", 'Product created successfully.');
                ...
                <div class="flex flex-col md:col-span-3 xl:col-span-4 gap-6 bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <DropdownApiSearchField :label="'Category'" :placeholder="'Enter category'" 
                            ...
                            @clearErrors="productStore.cleanErrors('product_category')"/>
                    </div>
